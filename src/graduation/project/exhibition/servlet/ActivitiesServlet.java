...
import graduation.project.exhibition.dao.ActivitiesDAO;
...
public class ActivitiesServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 利用DAO读取数据并返回
		ActivitiesDAO.get(new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new Activities());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new Activities());
				}
