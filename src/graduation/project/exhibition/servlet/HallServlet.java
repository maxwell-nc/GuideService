...
import graduation.project.exhibition.dao.HallDAO;
...
public class HallServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		final String spec = request.getParameter("spec");
		// 客户端请求的返回数据类型
		...
		// 利用DAO读取数据并返回
		HallDAO.get(spec,new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new Hall());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new Hall());
				}
