...
import graduation.project.exhibition.dao.ProductDetailDAO;
import graduation.project.exhibition.domain.ProductDetails;
...
public class ProductDetailServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 利用DAO读取数据并返回
		ProductDetailDAO.get(id,new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new ProductDetails());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new ProductDetails());
				}
