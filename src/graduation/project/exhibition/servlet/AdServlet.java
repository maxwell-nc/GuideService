...
import graduation.project.exhibition.dao.AdDAO;
import graduation.project.exhibition.domain.Ad;
...
public class AdServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 利用DAO读取数据并返回
		AdDAO.get(discoverType,new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new Ad());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new Ad());
				}
