...
import graduation.project.exhibition.dao.SearchDiscoverDAO;
...
@WebServlet("/search_discover")
// 对应的Servlet地址
public class SearchDiscoverServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public SearchDiscoverServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		// 利用DAO读取数据并返回
		SearchDiscoverDAO.get(key,new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new Discover());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new Discover());
				}
