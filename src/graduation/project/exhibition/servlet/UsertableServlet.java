...
import graduation.project.exhibition.dao.UsertableDAO;
import graduation.project.exhibition.domain.Usertable;
...
public class UsertableServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		// 获取用户名
		String comName = request.getParameter("username");
		// 客户端请求的返回数据类型
		...
		// 利用DAO读取数据并返回
		UsertableDAO.getUserInfo(comName, new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new Usertable());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new Usertable());
				}
