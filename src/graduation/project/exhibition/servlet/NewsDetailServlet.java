...
import graduation.project.exhibition.dao.NewsDetailDAO;
...
public class NewsDetailServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// 利用DAO读取数据并返回
		NewsDetailDAO.get(id,new SimpleOnGetResultSetListener() {
			...
			public void onGetResultSet(ResultSet rs) {
				...
				if (type != null && "xml".equals(type)) {
					// 把结果集存为XML
					returnString = XMLProcesser.putInXML(rs, new NewsDetail());
				} else {
					// 把结果集存为JSON
					returnString = JSONProcesser.putInJSON(rs, new NewsDetail());
				}
